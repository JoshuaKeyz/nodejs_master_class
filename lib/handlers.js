...
// Users - post
// Required data: firstName, lastName, password, tosAggrement
// Optional data: none
...
// Required data: phone
// Optional data: none 
// Only let an authenticated user access their object, don't let them access anyone else's object
...
// @TODO Only let an authenticated user delete their object. Don't let let them delete anyone else's
				...
				_data.read('users', phone, function(err, data){
					if(!err && data){
						_data.delete('users', phone, function(err){
							if(!err){
								//callback(200);
								// Deletes each of the checks associated with the user
		...
		_data.read('tokens', id, function(err, tokenData){
			if(!err && tokenData){
				...
				if(tokenData.expires > Date.now()){
					...
					callback9400, {'Error': 'The token has already expired, and cannot be extended'}
				}
			}else{
...
// Checks - put
// Required data : id
// Optional data: protocol, url, method, successCode, timeoutSeconds (one must be present)
						...
						if(tokenIsValid){
							...
							_data.update('checks', id, checkData, function(err){
								if(!err){
									...
									callback(500, {'Error': 'Could not update the object'})
								}
							})
						}else{
							callback(403)
