...
handlers._users.get = function (data, callback) {
  ...
  const phone = typeof (data.queryStringObject.phone.trim()) === 'string' &&
...
// Don't let them delete anyone elses
                    ...
                    _data.delete('checks', checkId, function(err){
                      ...
                      if(checksDeleted === checksToDelete) {
                        if(!deletionErrors) {
                          ...
                          callback(500, {'Error': 'Errors encountered while attempting to delete all of the user\'s checks. All checks may not have been deleted from the successfully'})
                        }
                      }
                    });
                  })
...
handlers._tokens.get = function (data, callback) {
  // Check that the phone number is valid
  const id = typeof (data.queryStringObject.id.trim()) === 'string' &&
...
handlers._tokens.put = function (data, callback) {
  const id = typeof (data.payload.id.trim()) === 'string' &&
        ...
        _data.read('users', userPhone, function (err, userData) {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              // Create the check object, and inclue the user's phone
              const checkObject = {
                'id': checkId,
                'userPhone': userPhone,
                'protocol': protocol,
              ...
              _data.create('checks', checkId, checkObject, function (err) {
                if (!err) {
                  ...
                  callback(500, { 'Error': 'Could not check the new check' });
                }
              });
            } else {
              ...
              if (!err) {
                ...
                _data.read('users', checkData.userPhone, function (err, userData) {
                  if (!err && data) {
                    ...
                    callback(500, { 'Error': 'Could not find the user who created the check, so could not delete the check from the user object' });
                  }
                });
              } else {
