...
// Perform the check, send the originalCheckData and the outcome of 
// the check process to the next in the process
workers.performCheck = (originalCheckData)=> {
  ...
  const parsedUrl = url.parse(originalCheckData.protocol+ '://' + 
...
// special logic for accomodating a check has never been tested 
// before (don't alert on that one)
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
